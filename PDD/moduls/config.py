...
class MyConf (RawConfigParser):
  ...
  def control_file (self):
    ...
    if not self.has_option ('FILES',    'db'):        self.set ('FILES',    'db',        PWD + '/databases/PDD.db')
    if not self.has_option ('FILES',    'tmp'):       self.set ('FILES',    'tmp',       PWD + '/now.jpg')
    if not self.has_option ('FILES',    'log'):       self.set ('FILES',    'log',       PWD + '/logs/PDD.log')
